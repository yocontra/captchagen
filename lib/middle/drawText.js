...
module.exports = function(canvas, opt) {
  ...
  var x = opt.offset ? opt.offset : 3;

  opt.text.split('').forEach(function(letter, idx) {
    var color = colors[idx];

    // set font
    var size = opt.fontsize ? opt.fontsize : getFontSize(opt.height, opt.width, opt.font);
    ...
    // set color
    ctx.fillStyle = opt.textColor ? opt.textColor : color.css;
